...
class AdminProductBannerScreen extends StatefulWidget {
  const AdminProductBannerScreen({Key? key}) : super(key: key);

  @override
  State<AdminProductBannerScreen> createState() => _AdminProductBannerScreenState();
}

class _AdminProductBannerScreenState extends State<AdminProductBannerScreen> {
...
//
  ...
  CollectionReference userss = FirebaseFirestore.instance.collection('product_banner');
  late ProductBanner homeBanner;
  ...
  updateData()async{
    homeBanner = ProductBanner(
      firstImgUrl: imageUrlWhite1,
      firstImgType: "1",
      secImgUrl: imageUrlwhite2,
      secImgType: "2",
    );
    FirebaseFirestore.instance.collection('product_banner').doc(refId).update(homeBanner.toJson());
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Product Screen Banner"),),
